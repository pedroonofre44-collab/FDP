        ...
        async for msg in ws:
            if msg.type == aiohttp.WSMsgType.TEXT:
                ...
                # BID
                    ...
                    # Last bidder: sum of all bids (including this one) must not equal nc
                    is_last_bidder = (G.bid_idx == len(alive) - 1)
                    if is_last_bidder:
                        current_sum = sum(G.bids.values()) + bid
                        if current_sum == nc:
                            forbidden = nc - sum(G.bids.values())
                            await send_error(ws, f'As last bidder you cannot bid {forbidden} (total would equal {nc}). Bid something else.'); continue
...
<div class="screen" id="s-game">
    ...
    <div class="panel" style="margin-bottom:1rem">
      ...
      <div style="display:flex;gap:.75rem;align-items:flex-end;flex-wrap:wrap;margin-top:.5rem">
        <div>
          ...
          <div class="info-note" id="bid-last-note" style="display:none"></div>
  ...
  <div id="play-ui" style="display:none">
    <!-- Last trick (shown after current trick resolves) -->
    ...
    <!-- Your hand — always visible during playing phase -->
...
function renderGame(){
  ...
  if(isMyBid){
    ...
    if(isLast){
      ...
      noteEl.textContent = `As last bidder you also cannot bid ${forbidden2} (total would equal ${nc})`;
