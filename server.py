...
.last-trick-area{background:var(--surface);border:1px solid var(--border2);border-radius:var(--r);
  padding:.75rem 1rem;margin-bottom:1rem;display:flex;gap:.75rem;flex-wrap:wrap;align-items:center}
.last-trick-label{font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.05em;margin-bottom:.4rem}
...
.info-note{font-size:12px;color:var(--muted);margin-top:.35rem;font-style:italic}
...
<div class="screen" id="s-lobby">
  <div class="top-bar">
    ...
    <span><span class="conn-dot on" id="cdot"></span>&nbsp;<span style="font-size:13px;color:var(--muted)">Connected</span></span>
  </div>
  <div class="lobby-wrap">
    ...
    <div class="panel">
      ...
      <ul style="font-size:13px;color:var(--muted);line-height:1.9;padding-left:1rem">
        ...
        <li>Bid tricks — never equal to cards in hand</li>
        <li>Total bids can never equal card count either</li>
...
<div class="screen" id="s-game">
  ...
  <div id="bid-ui" style="display:none">
    <div class="panel" style="margin-bottom:1rem">
      ...
      <div style="display:flex;gap:.75rem;align-items:flex-end;flex-wrap:wrap;margin-top:.5rem">
        <div>
          <div style="font-size:13px;color:var(--muted);margin-bottom:.3rem">Your bid (0 to <span id="max-bid-num"></span>)</div>
          <div class="info-note" id="bid-last-note" style="display:none;color:#e74c3c;font-style:normal;font-size:13px;margin-top:4px"></div>
          <input type="number" id="bid-input" min="0" style="width:100px;margin-top:.4rem">
        </div>
  ...
  <!-- Table — always visible once game starts -->
  <div id="trick-area" style="display:none">
    <!-- Last trick -->
    <div id="last-trick-wrap" style="display:none">
      <div class="last-trick-label">Last trick</div>
      <div class="last-trick-area" id="last-trick-area"></div>
    </div>

    <h3 style="margin-bottom:.5rem">Table — current trick</h3>
    <div class="table-area" id="table-area"><span style="color:var(--dim);font-size:13px">No cards played yet</span></div>
  </div>

  <!-- Playing UI — hand + action -->
  <div id="play-ui" style="display:none">
    <div id="play-hand-section">
      <h3 style="margin-bottom:.5rem">Your hand</h3>
      ...
      <div id="play-action">
        ...
        <span id="waiting-play" class="waiting" style="display:none;margin-left:.75rem"><span>Waiting for others</span><span class="dot-blink"></span></span>
      </div>
    </div>
  </div>
...
function renderLobby(){
  ...
  if(isHost){
    sa.innerHTML=`<button class="btn btn-gold" onclick="startGame()">Start game</button>
      <div style="font-size:12px;color:var(--dim);margin-top:.4rem">You are the host</div>`;
  } else {
...
function renderGame(){
  ...
  // Sequence track
  let seqH='';
  s.round_seq.forEach(r=>seqH+=`<div class="seq-pip ${r.idx<s.round_idx?'done':r.idx===s.round_idx?'active':''}">${r.nc}</div>`);
  ...
  const sumBids=Object.values(s.bids).reduce((a,b)=>a+b,0);
  ...
  }).join('')+`<div class="bid-box"><div class="bid-pname">Total bids</div><div class="bid-val">${sumBids} / ${nc}</div></div>`;
  ...
  document.getElementById('trick-area').style.display=ph==='playing'?'block':'none';
  document.getElementById('play-ui').style.display=ph==='playing'?'block':'none';

  if(isMyBid){
    document.getElementById('max-bid-num').textContent=nc;
    ...
    const noteEl = document.getElementById('bid-last-note');
    if(isLast){
      const forbidden2 = nc - sumBids;
      noteEl.textContent = `⚠ You are last bidder — cannot bid ${forbidden2} (total would equal ${nc})`;
  ...
  if(ph==='playing'){
    // Last trick display
    const ltw=document.getElementById('last-trick-wrap');
    const lta=document.getElementById('last-trick-area');
    if(s.last_trick && s.last_trick.length>0){
      ltw.style.display='block';
      let best=Infinity,winP=null;
      s.last_trick.forEach(e=>{const r=cRank(e.card,s.card_order);if(r<best){best=r;winP=e.player;}});
      lta.innerHTML=s.last_trick.map(e=>{const isW=e.player===winP;
        return `<div class="table-entry${isW?' winner-entry':''}">
          <div class="table-pname">${esc(e.player)}${isW?' ★':''}</div>
          <div class="table-card ${SUIT_CLS[e.card.s]}">${e.card.v}${SUIT_SYM[e.card.s]}</div>
        </div>`;}).join('');
    } else {
      ltw.style.display='none';
    }
    ...
    renderHand(s.my_hand,'play-hand',isMyPlay);
    document.getElementById('btn-play').style.display=isMyPlay?'inline-flex':'none';
    document.getElementById('btn-play').disabled=selectedCardIdx===null;
    document.getElementById('waiting-play').style.display=isMyPlay?'none':'inline-flex';
  }
...
function renderHand(hand,elId,selectable){
  const el=document.getElementById(elId);
  if(!hand||!hand.length){el.innerHTML='<span style="color:var(--dim)">No cards</span>';return;}
  el.innerHTML=hand.map((c,i)=>`<div class="card ${SUIT_CLS[c.s]}${selectable&&selectedCardIdx===i?' selected':''}"
    onclick="${selectable?`selectCard(${i})`:''}" style="${selectable?'':'cursor:default'}">${c.v}<div class="card-suit-sm">${SUIT_SYM[c.s]}</div></div>`).join('');
}

function selectCard(i){
  selectedCardIdx=i;
  renderHand(state.my_hand,'play-hand',true);
...
function showToast(msg,color=''){
  ...
  t.style.display='block'; clearTimeout(toastTimer); toastTimer=setTimeout(()=>t.style.display='none',4000);
}
